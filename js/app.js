...
const BADGES = [
    ...
    { id: "ps-foundation",     name: "PowerShell Foundation", icon: "⚡", desc: "Complete Introduction & Lab Setup" },
    { id: "dbatools-pro",      name: "DBATools Pro",       icon: "🛠️", desc: "Complete the DBATools module" },
    { id: "backup-master",     name: "Backup Master",      icon: "💾", desc: "Complete Backups & Restore" },
    { id: "security-guardian", name: "Security Guardian",  icon: "🛡️", desc: "Complete Security Management" },
    { id: "monitoring-ace",    name: "Monitoring Ace",     icon: "📊", desc: "Complete the Grafana Monitoring Dashboard module" },
    { id: "ag-architect",      name: "Always-On Architect", icon: "🏛️", desc: "Complete Always On Availability Groups" },
...
function checkModuleBadges() {
    const progress = getProgress();
    const moduleMap = {
        0:  "ps-foundation",      // Module 1: Introduction & Lab Setup
        1:  "dbatools-pro",       // Module 2: DBATools Module
        5:  "backup-master",      // Module 6: Backups & Restore
        6:  "security-guardian",  // Module 7: Security Management
        9:  "monitoring-ace",     // Module 10: Grafana Monitoring Dashboard
        12: "ag-architect"        // Module 13: Always On Availability Groups
    };
...
function renderSection() {
    ...
    // Update file badge
    if (section.examples && section.examples.length > 0) {
        document.getElementById('fileBadge').textContent = '🔒 ' + section.examples[0].name + '.ps1';
    } else {
        document.getElementById('fileBadge').textContent = '📄 scratch.ps1';
    }
...
// ============================================================
// HYBRID EXECUTION (PowerShell)
//   Translate safe PowerShell ? Python and run via Pyodide.
//   Anything outside the whitelist falls back to a friendly
//   "Copy to terminal" instruction.
// ============================================================
...
async function runCode() {
    ...
    if (!r.ok) {
        output.textContent =
            "?? This script needs real PowerShell.\n" +
            "Reason: " + r.reason + "\n\n" +
            "Click 'Copy' and paste it in your PowerShell terminal.";
        return;
    }

    output.textContent = "PS C:\\Lab> .\\example.ps1\n";
    ...
    try {
        ...
        await pyodide.runPythonAsync(r.python);
        if (r.warnings && r.warnings.length > 0) {
            output.textContent += "\n? Some lines were skipped in this preview:\n";
            r.warnings.forEach(w => {
                output.textContent += "  Line " + w.line + ": " + w.text + "\n";
            });
        }
        trackCodeRun();
    } catch (error) {
...
async function runSelected() {
    ...
    const r = window.PsTranslator.translate(selected);
    if (!r.ok) {
        output.textContent =
            "?? This selection needs real PowerShell.\n" +
            "Reason: " + r.reason + "\n\n" +
            "Click 'Copy' and paste it in your PowerShell terminal.";
        return;
    }

    output.textContent = "PS C:\\Lab> # selected lines\n";
...
function copyCode() {
    ...
    navigator.clipboard.writeText(code).then(() => {
        showToast('\u{1F4CB} Code copied � paste it in your PowerShell terminal.');
    });
}
...
async function sendChat() {
    ...
    const chatPrompt = `You are a helpful PowerShell tutor for SQL Server DBAs. You answer questions about PowerShell, SQL Server administration, dbatools, and the script shown below. If a question is unrelated, politely decline.
...
Give a clear, helpful answer in 4-6 sentences. If the question is about the code, refer to specific lines and cmdlets. If it's a broader PowerShell or SQL Server admin question relevant to the course, answer it directly.`;
        ...
        try {
            ...
            const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
                ...
                headers: {
                    ...
                    "X-Title": "PowerShell for SQL Server DBAs - Interactive Course"
                },
                body: JSON.stringify({
                    model: model,
